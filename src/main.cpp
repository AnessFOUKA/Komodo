...
int main(){
    ...
    g1.getMemoryManager()->addScript("testb",new Script([&g1,&c1](Script* script){
        if(script->checkCondition(0.0f)){
            ...
            g1.addItem(new AnimatedImage("app0:/assets/test.png",0,0,{{0,0,400,300},{0,0,200,300},{0,0,200,300}},0,1,2,2,{},{}),static_cast<ItemHandler*>(script->getMother()));
        }
        if(script->checkCondition(1.0f)){
            g1.removeItem(0,static_cast<ItemHandler*>(script->getMother()));
            g1.getMemoryManager()->removeImg("app0:/assets/test.png");
        }
